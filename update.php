...
if ($type == 1) {
    $sql = "UPDATE `main` SET `date`= ?,`title`= ?,`amount`= ?,`spending_category`= ?,`type`= ?,`memo`= ?,`created_at`= CURRENT_TIME() WHERE `main_id` = ?;";
} elseif ($type == 2) {
    $sql = "UPDATE `main` SET `date`= ?,`title`= ?,`amount`= ?,`income_category`= ?,`type`= ?,`memo`= ?,`created_at`= CURRENT_TIME() WHERE `main_id` = ?;";
}
...
$stmt->bindValue(7, $main_id, PDO::PARAM_STR);
